...
function getKeyboardFromButtonSet(buttonSet) {
  ...
  for (let i = 0; i < buttonSet.length; i += 1) {
    ...
    keyboardRow.className = 'virtual-keyboard_row';
  ...
  switchLang(lang) {
    ...
    for (let i = 0; i < buttons.length; i += 1) {
      ...
      if (typeof this.buttonSet[buttons[i].value].key === 'object'
        ...
        buttons[i].querySelector('.btn_stdkey').textContent = this.buttonSet[buttons[i].value].key[lang];
      }
      // Set altkey
      if (this.buttonSet[buttons[i].value].altkey) {
        ...
        if (this.buttonSet[buttons[i].value].altkey[lang] === null) {
          buttons[i].querySelector('.btn_altkey')?.remove();
        } else {
          // If altkey span was removed
          if (!buttons[i].querySelector('.btn_altkey')) {
            buttons[i].insertAdjacentHTML('afterbegin', '<span class="btn_altkey"></span>');
          }
          // Set altkey
          ...
          buttons[i].querySelector('.btn_altkey').textContent = altkey;
        }
      }
    }
  }
